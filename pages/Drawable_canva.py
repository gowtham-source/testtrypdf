...
def full_app():
    ...
    * In polygon mode, left-click to add a point, right-click to close the polygon, double-click to remove the latest point
    ...
    bg_color = st.sidebar.color_picker("Background color hex: ", "#e8d2a9")
...
def color_annotation_app():
    ...
    with st.echo("below"):
        ...
        mode = "transform" if st.sidebar.checkbox(
            "Move ROIs", False) else "rect"
        ...
        if canvas_result.json_data is not None:
            ...
            st.dataframe(
                df[["top", "left", "width", "height", "fill", "label"]])
    ...
    with st.echo("below"):
        ...
        if (
            ...
            for ind, path in enumerate(paths):
                path = parse_path(" ".join([str(e)
                                  for line in path for e in line]))
                st.write(f"Path {ind} has length {path.length():.3f} pixels")


if __name__ == "__main__":
    # st.set_page_config(
    #     page_title="Drawable Canvas", page_icon=":pencil2:"
    # )
